function generateRewardPage(root,cb){
    root.innerHTML = ''
    root.innerHTML += `
    <div class="card">
        <div class="card-body">
        ...
        <div class="absolute-menu-wrapper">
            <div class="absolute-menu">
                <p class="absolute-title">Yangi  taqdirlash</p>
...
function rewardPageFuncs(){
    let imgInputFile = [];
    function getRewardes(){
        ...
        .then(data => data.forEach((item, i) => {
            ...
            <tr data-id="${item._id}" data-img="${item.img}" data-title="${item.title}" data-text="${item.text}">
                <td>${i+1}</td>
                <td><img src="/${item.img}" alt="taqdirlash_rasmi"></td>
